...
//Make sure to call initializeOpenGLBuffers before making a call to draw!!
OBJObject::OBJObject(const char *objFileName, const OBJObjectShaderHandles &shaderHandles, const GLJoe::Transform &cMw,
                    const GLJoe::Transform &wMo, OBJObjectParams *defaultShaderParams): shaderHandles(shaderHandles),
                    cMw(cMw), wMo(wMo), buffersInitialized(false)
{
    ...
    if (!mesh->mModel->nummaterials) {
        if (!defaultShaderParams) {
            std::cout<<"Error! No default shader parameters specified! Aborting!";
            exit(1);
        }
